...
2. ไปที่ **SQL Editor** แล้วรันไฟล์ตามลำดับ:
   - `supabase/migrations/0001_init.sql` (ตาราง + enum + storage bucket)
   - `supabase/migrations/0002_rls.sql` (Row Level Security)
   - `supabase/seed.sql` (อาคาร + แม่แบบงาน 1–10 + งานทดสอบ)
...
1. push โค้ดขึ้น GitHub แล้ว import ที่ Vercel
2. ตั้ง Environment Variables ให้ครบตาม `.env.example`
   (ตั้ง `APP_BASE_URL` เป็น URL ของ Vercel)
3. เพิ่ม `https://<your-app>.vercel.app/auth/callback` ใน Supabase Auth redirect URLs
